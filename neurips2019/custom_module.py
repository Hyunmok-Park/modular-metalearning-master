...
class torch_NN(nn.Module):
  ...
  def __init__(self, inp=1, out=1, hidden=[], final_act='affine', loss_fn=None):
    ...
    def module_from_name(name):
      ...
      if name == "GRU":
        return nn.GRUCell(input_size=hidden[0], hidden_size=hidden[0])
      else:
        num = int(name[-1])
        typ = name[:-1]
        if typ == 'fc':
    ...
    elif self.final_act == 'GRU':self.add_module('fa', nn.Sequential())
    ...
    elif self.final_act == 'softmax':self.add_module('fa', nn.LogSoftmax(dim=-1))
    else: raise NotImplementedError
  ...
  def forward(self, x, weights=None, prefix=''):
    ...
    '''
    if weights is None:
      if "GRU" in str(self.features):
        x = self.features[0](x[0], x[1])
        x = self.fa(x)
      else:
        x = self.features(x)
        x = self.fa(x)
