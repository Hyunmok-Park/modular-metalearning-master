...
class HGNN_Composer(Composer):
  def __init__(self, composer, module_list, loss_fn=None, structure={},
    ...
    self.output_modules = self.module_list[-1]
  ...
  def forward_no_weights(self, b, J=None, mini_batch_size=None, iter=None, i_batch=None): #mine
    steps = self.structure['self_regress_steps']
    self.bs = b.shape[0]  # batch_size
    self.n = b.shape[1]  # number of nodes
    # incoming messages?
    state = torch.zeros(self.bs, self.msg_sz).cuda()
    ## Pointing tensors ##
    # edge_sources = self.structure['edge_sources']  # module->[list of nodes]
    # edge_sinks = self.structure['edge_sinks']  # module->[list of nodes]
    # node_idx = self.structure['node_idx']  # module->[list of nodes]

    edge_sources = [J[0].T[:,0].tolist()]
    edge_sinks = [J[0].T[:,1].tolist()]
    ...
    # cc = copy.deepcopy(node_idx)
    #
    # if mini_batch_size is not None:
    #   for _ in range(mini_batch_size-1):
    #     edge_sources[0] += [__ + (_+1)*100 for __ in aa]
    #   for _ in range(mini_batch_size-1):
    #     edge_sinks[0] += [__ + (_+1)*100 for __ in bb]
    #   for _ in range(len(self.node_modules)):
    #     for __ in range(mini_batch_size-1):
    #       node_idx[_] += [___ + (__+1)*100 for ___ in cc[_]]

    # self.structure['edge_sources'] = edge_sources
    # self.structure['edge_sinks'] = edge_sinks
    # self.structure['node_idx'] = node_idx
    ...
    # import ipdb; ipdb.set_trace()
    # create one-hot feature embedding of edge sinks?
    for sinks in edge_sinks:
      if sinks == []:
        self.structure['sinks_one_hot'].append(None)
        continue
      # z = torch.zeros(self.structure['num_nodes'], len(sinks)).cuda()
      z = torch.zeros(self.structure['num_nodes'] * mini_batch_size, len(sinks)).cuda() if mini_batch_size is not None else torch.zeros(self.structure['num_nodes'], len(sinks)).cuda()
      aux_idx = torch.cuda.LongTensor(np.array(sinks)).reshape(-1, len(sinks))
      self.structure['sinks_one_hot'].append(z.scatter_(dim=0, index=aux_idx, value=1.).cuda())
      # print('1-hot: ', self.structure['sinks_one_hot'][-1])

    for step in tqdm(range(steps), desc="Propagation", leave=False):
      # Edge modules: concat input from sources and sinks and feed through edge module
      for i, module in enumerate(self.edge_modules):
        if len(edge_sources[i]) == 0: continue
        J_ij = J[1].view(-1, 1)

        # J_ij = J[edge_sources[i], [_ for _ in edge_sinks[i]]].view(-1, 1)
        # no_edge_idx = np.where(J_ij.cpu() != 0)[0]
        # no_edge_idx = torch.nonzero(J_ij)[:,0]
        # J_ij = J_ij[no_edge_idx]

        # sources = b[np.array(edge_sources[i])[no_edge_idx], :].clone()
        # sinks = b[np.array(edge_sinks[i])[no_edge_idx], :].clone()
        # state_sources = state[np.array(edge_sources[i])[no_edge_idx], :].clone()
        # state_sinks = state[np.array(edge_sinks[i])[no_edge_idx], :].clone()
        sources = b[edge_sources[i], :]
        sinks = b[edge_sinks[i], :]
        state_sources = state[edge_sources[i], :]
        state_sinks = state[edge_sinks[i], :]

        ff_in = torch.cat([sources, -sources, J_ij, -J_ij], dim=-1).float()
        ff_out = torch.cat([-sinks, sinks, -J_ij, J_ij], dim=-1).float()
        # concat messages from source and sink to form edge state
        inp = torch.cat([state_sources, ff_in, state_sinks, ff_out], dim=-1)
        # feed through edge module
        out = module(inp)
        # incoming = torch.matmul(self.structure['sinks_one_hot'][i][:, no_edge_idx], out)

        incoming = torch.matmul(self.structure['sinks_one_hot'][i], out)
      ...
      for (i, module) in enumerate(self.node_modules):
        if len(node_idx[i]) == 0: continue
        msg = incoming[node_idx[i], :]
        state_prev = state[node_idx[i], :]
        # Updates node's value; no worries about updating too early since each node only affects itself.
        if "GRU" in str(module):
          aux = module([msg, state_prev])
        else:
          aux = module(msg, state_prev)

        state[node_idx[i], :] = aux

    output = self.output_modules(state)
