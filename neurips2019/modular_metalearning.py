...
class BounceGrad(object):
  def __init__(self, S, args):
    ...
    self.everys = [1, 10, 20]
    self.everys_train = [i for i in range(50 * self.S.graph['num_nodes'])]
    self.everys_val = [i for i in range(10 * self.S.graph['num_nodes'])]
  ...
  def initialize_modules(self):
    ...
    ### add readout function ###
    l.append(len(self.L))
    self.L.append(torch_NN(inp=64,out=2,hidden=[64,64],final_act="softmax").to(device=self.nn_device))
    self.S.Modules.append(l)
    ...
    self.SOpt = torch.optim.Adam(self.L.parameters(), lr=self.meta_lr, weight_decay=0.0)
    self.SOpt_scheduler = torch.optim.lr_scheduler.MultiStepLR(self.SOpt,milestones='10000',gamma=0.1)
  ...
  def initialize_dataset(self, data_desc):
    ...
    if self.plot_with_without_node:
      ...
      self.DL_Mtrain, self.DL_Mval, self.DL_Mtest \
        = data_loading.get_data_loaders(self.T, batch_size=self.meta_batch_size, option_batch=self.num_test)
  ...
  def run_model(self, structure, inp, inp_J = None, instructions={}, mini_batch_size=None, iter=None, i_batch=None):
    '''
    ...
    return self.S.composer_class(composer=self.composer,
        module_list=self.L, structure=structure,instructions=instructions)(inp, inp_J, mini_batch_size=mini_batch_size, iter=iter, i_batch=i_batch)

  def batched_evaluate(self, structures, datasets, mode, step, i_batch):
    '''
    ...
    if self.S.find_node:
      ...
      # inp_J = diag_block_mat_slicing([dataset.TrainInput_J if mode == 'Train'
      #            else dataset.ValInput
      ...
      if mode == "Train":
        train_step = step % 40
        iter = 0
        Train_data_list = [Data(x=x, edge_index=J[0], edge_attr=J[1], y=o) for x, J, o in zip(datasets[0].TrainInput, datasets[0].TrainInput_J, datasets[0].TrainOutput)]
        Train_loader = DataLoader(Train_data_list[50*train_step:50*(train_step+1)], batch_size=self.mini_batch_size, shuffle=True)
        outs = datasets[0].TrainOutput[50*train_step:50*(train_step+1)]
        for mini_batch in tqdm(Train_loader, desc="TRAIN MINIBATCH", leave=False):
          inp = mini_batch['x']
          inp_J = [mini_batch['edge_index'], mini_batch['edge_attr']]

          # inp_J = dense_to_sparse(diag_block_mat_slicing(datasets[0].TrainInput_J[mini_batch*self.mini_batch_size:(mini_batch+1)*self.mini_batch_size])[0])
          # inp_J = datasets[0].TrainInput_J[mini_batch * self.mini_batch_size:(mini_batch + 1) * self.mini_batch_size].view(-1, self.S.graph['num_nodes'])
          ...
          preds = self.run_model(mega_structure, inp, inp_J, mini_batch_size=self.mini_batch_size, iter=iter, i_batch=i_batch)
          preds_list.append(preds)
          iter+=1

      elif mode == "Test":
        iter = 0
        Test_data_list = [Data(x=x, edge_index=J[0], edge_attr=J[1], y=o) for x, J, o in
                           zip(datasets[0].TrainInput, datasets[0].TrainInput_J, datasets[0].TrainOutput)]
        Test_loader = DataLoader(Test_data_list, batch_size=1, shuffle=True)
        outs = torch.cat([dataset.TrainOutput for dataset in datasets], dim=0)
        for mini_batch in tqdm(Test_loader, desc="TEST MINIBATCH", leave=False):
          inp = mini_batch['x']
          inp_J = [mini_batch['edge_index'], mini_batch['edge_attr']]

          preds = self.run_model(mega_structure, inp, inp_J, mini_batch_size=self.mini_batch_size, iter=iter)
          preds_list.append(preds)
          iter += 1
        ...
        iter = 0
        val_step = step % 10
        Val_data_list = [Data(x=x, edge_index=J[0], edge_attr=J[1], y=o) for x, J, o in
                         zip(datasets[0].ValInput, datasets[0].ValInput_J, datasets[0].ValOutput)]
        Val_loader = DataLoader(Val_data_list[10*val_step:10*(val_step+1)], batch_size=self.mini_batch_size, shuffle=True)
        outs = datasets[0].ValOutput[10*val_step:10*(val_step+1)]
        for mini_batch in tqdm(Val_loader, desc="VAL MINIBATCH", leave=False):
          inp = mini_batch['x']
          inp_J = [mini_batch['edge_index'], mini_batch['edge_attr']]
    ...
    if mode == "Test":
      ...
      preds_list = preds_list.view(self.meta_batch_size, -1, 2)
      outs = outs.view(self.meta_batch_size, -1, 2)

      loss = [abs(nn.KLDivLoss(reduction='batchmean')(pred, out)) for pred, out in zip(preds_list, outs)]
      loss_every = [[abs(torch.sum(nn.KLDivLoss(reduction='none')(pred, out))) for pred, out in zip(p,o)] for p,o in zip(preds_list, outs)]

      return loss, loss_every, torch.exp(preds_list.view(self.meta_batch_size, -1, 2))

    else:
      preds_list = torch.cat(preds_list, dim=0)
      # preds_list = (preds_list+eps)/(preds_list+eps).sum(axis = 1).view(-1,1)
      outs = outs.view(-1,2)
      loss = abs(nn.KLDivLoss(reduction='batchmean')(preds_list, outs))
      loss_every = abs(torch.sum(nn.KLDivLoss(reduction='none')(preds_list, outs), dim=-1))
      return [loss], [loss_every], torch.exp(preds_list.view(self.meta_batch_size, -1, 2))
    ...
    # preds = self.run_model(mega_structure, inp, inp_J)
    # preds = preds.squeeze()  ###mine###
  ...
  def batched_bounce(self, batch_structures, batch_datasets, temp, do_grad, step=None, mode=None, i_batch=None):
    '''
    ...
    if self.encoder_input_type not in ["truth"]:
      ...
      orig_train_loss, orig_train_loss_every, orig_train_pred = \
          self.batched_evaluate(batch_structures, batch_datasets, 'Train' if mode is None else mode, step=step, i_batch=i_batch)
      ...
      if batch_datasets[0].TrainInput.shape != batch_datasets[0].ValInput.shape \
        ...
        orig_val_loss, orig_val_loss_every, orig_val_pred = \
            self.batched_evaluate(batch_structures, batch_datasets, 'Val' if mode is None else mode, step=step, i_batch=i_batch)
      else:
        orig_val_loss, orig_val_loss_every, orig_val_pred = \
            orig_train_loss, orig_train_loss_every, orig_train_pred
    ...
    new_train_loss, new_train_loss_every, new_train_pred = \
        self.batched_evaluate(proposed_structs, batch_datasets, 'Train' if mode is None else mode, step=step, i_batch=i_batch)

    if batch_datasets[0].TrainInput.shape != batch_datasets[0].ValInput.shape \
       or torch.any(batch_datasets[0].TrainInput != batch_datasets[0].ValInput):
      new_val_loss, new_val_loss_every, new_val_pred = self.batched_evaluate(
          proposed_structs, batch_datasets, 'Val' if mode is None else mode, step=step, i_batch=i_batch)
    else:
  ...
  def SAConfig_SGDModules(self, optimization_steps):
    ...
    for step in tqdm(range(optimization_steps), desc="OPTIMIZATION"):

      my_train_loss_batch = []
      my_val_loss_batch = []
      my_test_loss_batch = []

      print("==================== Opt step : {} / {} ====================".format(step+1, optimization_steps))
      self.step = step
      self.S.initialize_all_structures(T=self.T)

      if self.do_mtrain and step and step % 10 ==0:
        # self.SOpt_scheduler.step(self.mean_current_val)  # MTrain-Val
        self.SOpt_scheduler.step()
        # if self.LocalOpt is not None:
      ...
      tik_val = time.time()
      if step % 10 == 0:
        for i_batch, batch_datasets in tqdm(enumerate(self.DL_Mval), desc="VAL", leave=False):
          # Update structure
          # for dataset in batch_datasets:
          ...
          updated_structures, train_loss, val_loss, train_loss_every, \
              ...
              accepted_structs, accepted_pred_probs, numcorr_bidir, MAML_g \
            = self.batched_bounce(
                batch_structures, batch_datasets, temp, do_grad=False, step=step, mode="Test")

          my_test_loss_batch.append(train_loss_every)
        ...
        my_test_loss.append(my_test_loss_batch)
      tak_val = time.time()
      val_time = tak_val-tik_val

      
      # print("EPOCH {} : TRAIN TIME : {} / TEST TIME : {}".format(step, train_time, val_time))
      my_train_loss.append(my_train_loss_batch)
      my_val_loss.append(my_val_loss_batch)
      ...
      if step % 10 == 0:
        result_dict = {}
        # my_train_loss = np.array(my_train_loss)
        # my_val_loss = np.array(my_val_loss)
        # my_test_loss = np.array(my_test_loss)

        result_dict['train_loss'] = np.array(my_train_loss)
        result_dict['val_loss'] = np.array(my_val_loss)
        result_dict['test_loss'] = np.array(my_test_loss)

        print("Saving test result")
        with open(os.path.join(self.models_path, "test_result_{}.p".format(step)), "wb") as f:
    ...
    print("Saving test result")
    with open(os.path.join(self.models_path, "test_result.p"), "wb") as f:
      pickle.dump(result_dict, f)
  ...
  def initialize_mtrain_metrics_tracking(self):
    ...
    self.current_train = []
    self.current_val = []
    self.current_train_every = [[] for _ in self.everys_train]
    self.current_val_every = [[] for _ in self.everys_val]
  ...
  def _initialize_mval_metrics_tracking(self):
    self.current_Mtrain = []
    self.current_Meval = []
    self.current_Mtrain_every = [[] for _ in range(self.num_test)]
    self.current_Meval_every = [[] for _ in range(self.num_test)]
  ...
  def _update_mtrain_perf_tracking(self, train_loss, val_loss,
    ...
    for i, tle in enumerate(train_loss_every):
      self.current_train_every[i].extend(tle)

    for i, vle in enumerate(val_loss_every):
      self.current_val_every[i].extend(vle)
  ...
  def update_metrics(self):
    ...
    for i, e in tqdm(enumerate(self.everys_train), desc="EVERYS T TRAIN", leave=False):
    ...
    for i, e in tqdm(enumerate(range(self.S.graph['num_nodes'])), desc="EVERYS V TRAIN", leave=False):
    ...
    for i, e in tqdm(enumerate(self.everys_val), desc="EVERY T VAL", leave=False):
      # mean across all datasets
      if self.do_mtrain:
    ...
    for i, e in tqdm(enumerate(range(self.S.graph['num_nodes'])), desc="EVERY V VAL", leave=False):
